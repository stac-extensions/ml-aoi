...
.PHONY: install
install: install-run install-pip    ## alias for 'install-all' target
...
install-dev: install-run install-npm-remarklint	## install development and test dependencies
	...
	) || echo "No development requirements to install."
...
install-pkg: install-pip	## install application package dependencies
	...
	) || echo "No application requirements to install."
...
install-sys:	## install system dependencies and required installers/runners
	@test -f "$(APP_ROOT)/requirements-sys.txt" && ( \
